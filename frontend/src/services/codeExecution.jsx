...
const formatExecutionResult = (result, language) => {
  ...
  switch (result.status?.id) {
    ...
    case 5: // Time Limit Exceeded
      throw new Error(`Time limit exceeded. Your ${languages[language].name} code took too long to execute.`);
    
    case 6: // Compilation Error
      throw new Error(`Compilation error in ${languages[language].name}:\n${result.compile_output || ''}`);
    
    case 7: // Runtime Error (SIGSEGV)
      if (language === 'cpp' || language === 'c') {
        throw new Error(`Segmentation fault - check your memory access and pointers.`);
      }
      throw new Error(`Runtime error:\n${result.stderr || ''}`);
    
    case 8: // Runtime Error (SIGXFSZ)
      throw new Error(`Output limit exceeded - your program produced too much output.`);
    
    case 9: // Runtime Error (SIGFPE)
      throw new Error(`Floating point error - division by zero or similar issue.`);
    ...
    case 11: // Runtime Error (NZEC)
      ...
      throw new Error(`Program exited with non-zero code:\n${result.stderr || ''}`);
    
    case 12: // Runtime Error (Other)
      throw new Error(`Runtime error in ${languages[language].name}:\n${result.stderr || result.compile_output || 'Unknown error'}`);
    
    default:
      throw new Error(`Unknown error occurred while running ${languages[language].name} code.`);
  }
};
